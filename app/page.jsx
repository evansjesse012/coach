...
function TodaySessionCard({ plan, cardio, strength, onStartStrength, setTab, trainingPlan }) {
    ...
    return (<Card accent={allDone?C.green:C.accent} style={{marginBottom:16}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8}}>{daySessions.map((sess,i)=>{
        ...
        return (<div key={i} onClick={isStr&&!done&&sess.exercises?()=>{onStartStrength(sess);setTab('plan');}:(!done?()=>setTab('log'):undefined)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',background:done?C.green+'0A':C.elevated,borderRadius:12,border:`1.5px solid ${done?C.green+'44':accent+'30'}`,cursor:!done?'pointer':'default',transition:'all .15s'}}>
          ...
          <div style={{flex:1}}>
            <div style={{display:'flex',alignItems:'center',gap:6}}>{sess.priority&&<div style={{width:6,height:6,borderRadius:'50%',background:sess.priority==='red'?C.accent:C.yellow}}/>}<div style={{fontFamily:F.ui,fontWeight:600,fontSize:15,color:done?C.green:C.text}}>{sport.label}</div>{sess.zone&&!done&&<span style={{fontFamily:F.mono,fontSize:11,color:sport.color,background:sport.color+'15',borderRadius:6,padding:'1px 6px'}}>{sess.zone}</span>}</div>
            {sess.label&&!done&&<div style={{fontFamily:F.ui,fontSize:12,color:C.subtle,marginTop:2}}>{sess.label}{sess.targetIntensity?' · '+sess.targetIntensity:''}</div>}
  ...
  return (<Card accent={allDone?C.green:C.accent} style={{marginBottom:16}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><div><div style={{fontFamily:F.display,fontSize:18,fontWeight:700,color:allDone?C.green:C.accent}}>{allDone?'✓ Today complete':"Today's sessions"}</div><div style={{fontFamily:F.ui,fontSize:13,color:C.subtle,marginTop:1}}>{getDayName()}</div></div>{allDone&&<Pill color={C.green}>Done</Pill>}</div><div style={{display:'flex',flexDirection:'column',gap:8}}>{todayPlan.sessions.map((sess,i)=>{const isStr=sess.type==='strength';const sport=SPORT_META[sess.sport||'other'];const done=isStr?todayS.some(s=>s.name===sess.label||s.templateId===sess.templateId):todayC.some(w=>w.sport===sess.sport);const accent=done?C.green:(isStr?C.green:sport.color);return (<div key={i} onClick={isStr&&!done&&sess.exercises?()=>{onStartStrength(sess);setTab('plan');}:(!done?()=>setTab('log'):undefined)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',background:done?C.green+'0A':C.elevated,borderRadius:12,border:`1.5px solid ${done?C.green+'44':accent+'30'}`,cursor:!done?'pointer':'default',transition:'all .15s'}}><div style={{width:36,height:36,borderRadius:12,background:accent+'20',display:'flex',alignItems:'center',justifyContent:'center',}}>{done?<Icon name='check' size={17} color={C.green}/>:(isStr?<Icon name='dumbbell' size={17} color={accent}/>:<Icon name={sport.icon} size={17} color={accent}/>)}</div><div style={{flex:1}}><div style={{display:'flex',alignItems:'center',gap:6}}><div style={{fontFamily:F.ui,fontWeight:600,fontSize:15,color:done?C.green:C.text}}>{(SPORT_META[sess.type]||SPORT_META[sess.sport]||SPORT_META.other).label}</div>{sess.zone&&!done&&<span style={{fontFamily:F.mono,fontSize:11,color:accent,background:accent+'15',borderRadius:6,padding:'1px 6px'}}>{sess.zone}</span>}</div>{sess.label&&!done&&<div style={{fontFamily:F.ui,fontSize:12,color:C.subtle,marginTop:1}}>{sess.label}{sess.targetIntensity?' · '+sess.targetIntensity:''}</div>}{sess.notes&&!done&&<div style={{fontFamily:F.ui,fontSize:12,color:C.muted,marginTop:1,lineHeight:1.4}}>{sess.notes}</div>}</div>{!isStr&&sess.duration&&<div style={{fontFamily:F.mono,fontSize:12,color:C.muted}}>{fmtDur(sess.duration)}</div>}{isStr&&!done&&sess.exercises&&<div style={{fontFamily:F.ui,fontSize:13,fontWeight:600,color:C.green}}>Start →</div>}{!isStr&&!done&&<div style={{fontFamily:F.ui,fontSize:13,fontWeight:600,color:sport.color}}>Log →</div>}</div>);})}</div></Card>);
}
              ...
              return (<div key={si} style={{borderRadius:12,border:`1.5px solid ${brickDone?C.green+'44':C.yellow+'40'}`,overflow:'hidden',background:brickDone?C.green+'08':'transparent',marginBottom:si<daySessions.length-1?4:0}}>
                ...
                {sess.legs.map((leg,li)=>{const lSport=SPORT_META[leg.sport]||SPORT_META.other;const lDone=dayCardio.some(w=>w.sport===leg.sport);return(
                  <div key={li} style={{padding:'10px 12px',borderTop:`1px solid ${C.border}`}}>
                    ...
                    {leg.workout&&!lDone&&<div style={{fontFamily:F.ui,fontSize:12,color:C.text,marginTop:4,marginLeft:44,padding:'6px 10px',background:C.elevated,borderRadius:8,border:`1px solid ${C.border}`,lineHeight:1.6}}>{renderMd(leg.workout)}</div>}
                    {leg.notes&&!lDone&&<div style={{fontFamily:F.ui,fontSize:12,color:C.subtle,marginTop:3,marginLeft:44,lineHeight:1.5}}>{renderMd(leg.notes)}</div>}
                  </div>);})}
              ...
              <div onClick={isStr&&sess.exercises&&!done?()=>startStrength(sess):undefined} style={{display:'flex',alignItems:'flex-start',gap:12,padding:'12px 10px',borderRadius:12,cursor:isStr&&sess.exercises&&!done&&!isMissed?'pointer':'default',borderBottom:si<daySessions.length-1?`1px solid ${C.border}`:'none',background:done?C.green+'08':(isMissed?C.red+'06':'transparent')}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                    ...
                    <div style={{fontFamily:F.ui,fontWeight:600,fontSize:15,color:done?C.green:(isMissed?C.muted:C.text),textDecoration:isMissed?'line-through':'none'}}>{sport.label}</div>
                    {sess.zone&&!done&&<span style={{fontFamily:F.mono,fontSize:11,color:sport.color,background:sport.color+'15',borderRadius:6,padding:'1px 6px'}}>{sess.zone}{sess.targetIntensity?' · '+sess.targetIntensity:''}</span>}
                  ...
                  {sess.label&&!done&&<div style={{fontFamily:F.ui,fontSize:13,color:C.subtle,marginTop:2}}>{sess.label}</div>}
                  ...
                  {sess.workout&&!done&&<div style={{fontFamily:F.ui,fontSize:12,color:C.text,marginTop:6,padding:'8px 10px',background:C.elevated,borderRadius:8,border:`1px solid ${C.border}`,lineHeight:1.6}}>{renderMd(sess.workout)}</div>}
                  {sess.notes&&!done&&<div style={{fontFamily:F.ui,fontSize:12,color:C.subtle,marginTop:4,lineHeight:1.5}}>{renderMd(sess.notes)}</div>}
