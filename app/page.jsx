    ...
    return(<Card key={'brick-'+brick.id} onClick={()=>setSelectedBrick(brick)} style={{marginBottom:8,padding:0,borderColor:C.yellow+'40',cursor:'pointer',overflow:'hidden'}}>
      ...
      {legs.map((leg,li)=>{const ls=SPORT_META[leg.sport]||SPORT_META.other;return(
        <div key={li} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',borderTop:li>0?`1px solid ${C.border}`:'none'}}>
          <SportBadge sport={leg.sport} small/><div style={{flex:1,minWidth:0}}><div style={{fontFamily:F.ui,fontSize:14,color:C.text,fontWeight:500,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{leg.notes||ls.label}</div></div>
          <span style={{fontFamily:F.mono,fontSize:13,color:C.muted}}>{fmtDur(leg.duration)}</span>
        </div>);})}
    </Card>);
  }
  return(<Card key={i} onClick={()=>setSelectedWorkout(w)} style={{marginBottom:8,padding:'13px 16px',cursor:'pointer'}}><div style={{display:'flex',alignItems:'center',gap:12}}><SportBadge sport={w.sport||'other'} small/><div style={{flex:1,minWidth:0}}><div style={{fontFamily:F.ui,fontSize:15,color:C.text,fontWeight:500,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{w.notes||'—'}</div><div style={{display:'flex',gap:6,marginTop:2}}>{w.source==='healthkit'&&<span style={{fontFamily:F.ui,fontSize:11,fontWeight:600,color:C.cyan}}>Apple Health</span>}</div></div><div style={{fontFamily:F.display,fontSize:22,fontWeight:700,color:C.text,flexShrink:0}}>{fmtDur(w.duration)}</div></div></Card>);
