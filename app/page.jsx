...
function QuickCaptureSheet({ onClose, onLog, plan }) {
  const[input,setInput]=useState('');const[loading,setLoading]=useState(false);const[parsed,setParsed]=useState(null);const inputRef=useRef(null);const mountedRef=useRef(true);
  useEffect(()=>{const t=setTimeout(()=>inputRef.current?.focus(),100);return()=>{clearTimeout(t);mountedRef.current=false;};},[]);
  ...
  const handleSubmit=async()=>{if(!input.trim()||loading)return;setLoading(true);try{const r=await parseQuickCapture(input,callAI);if(mountedRef.current)setParsed(r);}catch{toast.error('Could not parse — try "45 min easy run"');}if(mountedRef.current)setLoading(false);};
  ...
  const handleSuggestion=async text=>{setInput(text);setLoading(true);try{const r=await parseQuickCapture(text,callAI);if(mountedRef.current)setParsed(r);}catch{}if(mountedRef.current)setLoading(false);};
