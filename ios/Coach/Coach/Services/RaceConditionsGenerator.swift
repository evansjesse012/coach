...
/// conditions in a structured JSON shape, then decodes that into AIConditions.
@MainActor
enum RaceConditionsGenerator {
    static func generate(for event: Event) async throws -> AIConditions {
        ...
        Return ONLY a JSON object with these fields, no markdown fences or commentary:
        {
          ...
          "tips": ["5-8 specific, actionable things an expert coach would tell their athlete about this exact race — pacing strategy, nutrition timing, gear choices, mental cues, terrain-specific advice"]
        }
        ...
        let body: [String: Any] = [
            "system": "You are an expert endurance coach with deep race-day knowledge. You return only valid JSON when asked.",
            "messages": [
        ...
        return try JSONDecoder().decode(AIConditions.self, from: jsonData)
    }
...
// MARK: - Minimal local Anthropic shape (we don't need the full one from AgentLoop)
